...
export class ApplicationService {
  ...
  async findAll(page?: number, size?: number): Promise<any> {
    return await lastValueFrom(this.httpClient.get(`${this.baseUrl.getUrlApplication()}findAll`));
  }
   ...
   async findByEmployerId(employerId: number, currentPage: number, status: number): Promise<any> {
    ...
    async countApplicantsByJobId(jobId: number): Promise<any> {
      return await lastValueFrom(this.httpClient.get(`${this.baseUrl.getUrlApplication()}countApplicantsByJobId/${jobId}`));
    }
    async historyApplication(seekerId: number, currentPage: number, size: number): Promise<any> {
      return await lastValueFrom(this.httpClient.get(`${this.baseUrl.getUrlApplication()}history-application?seekerId=${seekerId}&page=${currentPage}&size=${size}`));
    }

    async listInterviews(currentPage: number, size: number): Promise<any> {
      return await lastValueFrom(this.httpClient.get(`${this.baseUrl.getUrlApplication()}list-interviews?page=${currentPage}&size=${size}`));
    }
}